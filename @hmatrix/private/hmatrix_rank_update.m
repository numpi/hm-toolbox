...
%
% H = HMATRIX_RANK_UPDATE(H, U, V) computes an HODLR representation of the
...
if is_leafnode(H)
    ...
    n1 = H.A11.sz(2);
    if exist('nrm', 'var')
        H.A11 = hmatrix_rank_update(H.A11, U(1:m1,:), V(1:n1,:), nrm);
        H.A12 = hmatrix_rank_update(H.A12, U(1:m1,:), V(n1+1:end,:), nrm);
        H.A21 = hmatrix_rank_update(H.A21, U(m1+1:end,:), V(1:n1,:), nrm);
        H.A22 = hmatrix_rank_update(H.A22, U(m1+1:end,:), V(n1+1:end,:), nrm);
    else
        H.A11 = hmatrix_rank_update(H.A11, U(1:m1,:), V(1:n1,:));
        H.A12 = hmatrix_rank_update(H.A12, U(1:m1,:), V(n1+1:end,:));
        H.A21 = hmatrix_rank_update(H.A21, U(m1+1:end,:), V(1:n1,:));
