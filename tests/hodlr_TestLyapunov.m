...
for bs = [ 32, 256 ]
    ...
    A  = hodlr('banded', sA, 1);
    C = h^2 * hodlr('chebfun2', @(x,y) log(1 + abs(x - y)), [0, 1], [0, 1], n);
    
    % Solver 1: completely hodlr
    X = lyap(A, C);
    CheckTestResult(norm(A * X + X * A + C), '<', ...
        4 * norm(A) * norm(X) * hodlroption('threshold'), ...
    ...
    CheckTestResult(norm(A * X + X * A + C), '<', ...
        10 * 2 * norm(A) * norm(X) * hodlroption('threshold'), ...
        [ 'Lyapunov equation with hodlr coefficients ' ...
    ...
    CheckTestResult(norm(A * X + X * B + C), '<', ...
        2 * (norm(B) + norm(A)) * norm(X) * hodlroption('threshold'), ...
    ...
    CheckTestResult(norm(A * X + X * B + C), '<', ...
        10 * (norm(B) + norm(A)) * norm(X) * hodlroption('threshold'), ...
        [ 'Sylvester equation with hodlr coefficients and RHS' ...
