...
for bs = [ 32, 256 ]
    ...
    CheckTestResult(norm(A * X + X * A + C), '<', ...
        (2 * norm(A) * norm(X) + norm(C) ) * hodlroption('threshold'), ...
    ...
    CheckTestResult(norm(A * X + X * A + C), '<', ...
       20 * (2 * norm(A) * norm(X) + norm(C)) * hodlroption('threshold'), ...
        [ 'Lyapunov equation with hodlr coefficients ' ...
    ...
    CheckTestResult(norm(A * X + X * B + C), '<', ...
         ((norm(B) + norm(A)) * norm(X) + norm(C)) * hodlroption('threshold'), ...
    ...
    CheckTestResult(norm(A * X + X * B + C), '<', ...
        20 * ((norm(B) + norm(A)) * norm(X) + norm(C)) * hodlroption('threshold'), ...
        [ 'Sylvester equation with hodlr coefficients and RHS' ...
