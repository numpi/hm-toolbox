...
A = randn(n);
H = hss(A)';
CheckTestResult(norm(A' - full(H)), '<', hnrm(A) * tol, ...
...
H = hss('low-rank', U, V)';
CheckTestResult(norm(V * U' - full(H)), '<', hnrm(U*V') * tol, ...
...
H = hssA + hodlrB;
CheckTestResult(norm(A + B - full(H)), '<', hnrm(hssA + hssB) * tol, ...
...
CheckTestResult(norm(A + B - full(H)), '<', hnrm(B + A) * tol, ...
...
CheckTestResult(norm(U*V' + B - full(H)), '<', hnrm(B + U*V') * tol, ...
