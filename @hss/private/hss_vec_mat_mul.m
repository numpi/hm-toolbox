...
if (A.leafnode == 0)   % not a leaf
    ...
    g.gl = bottom_up(A.hssl, v(:, 1:A.ml));
    g.gr = bottom_up(A.hssr, v(:, A.ml+1:end));
...
if (A.leafnode==0)   % not a leaf
	...
	fr = g.gl.vU * A.Bu;
        ...
        fl = g.gr.vU * A.Bl + f * A.Wl';
        fr = g.gl.vU * A.Bu + f * A.Wr';
    end
   
    xl = up_bottom(A.hssl, v(:, 1:A.ml), g.gl, fl);
    xr = up_bottom(A.hssr, v(:, A.ml+1:end), g.gr, fr);
    x = [xl, xr];
