function [H] = halr_from_adaptive(Afun, m, n, maxrank, support, nrm)
%HALR_FROM_ADAPTIVE Construct an halr of a function sampling
%
% Inputs:
%
...
if isempty(maxrank) || maxrank > round(min(m,n) * halroption('rank-ratio'))
    maxrank = round(min(m,n) * halroption('rank-ratio'));
end
...
if (m <= bs && n <= bs) || min([m n]) == 1
        ...
        case 1
            ...
            switch compression_method
                ...
                case 'svd'
                    ...
                    U = U * S;
                    if size(U, 2) < min(maxrank, min(m, n) * halroption('rank-ratio'))
                        H.admissible = true;
                        H.U = U;
                        H.V = V;
                    else
    ...
    if ~H.admissible
        ...
        H.A11 = halr_from_adaptive(@(i,j) Afun(i, j), m1, n1, ...
            maxrank, support, nrm);
        H.A12 = halr_from_adaptive(@(i,j) Afun(i, j + n1), m1, ...
            n - n1, maxrank, ...
            @(i1, j1, i2, j2) support(i1, j1 + n1, i2, j2 + n1), nrm);
        H.A21 = halr_from_adaptive(@(i,j) Afun(i + m1, j), ...
            m - m1, n1, maxrank, ...
            @(i1, j1, i2, j2) support(i1 + m1, j1, i2 + m1, j2), nrm);
        H.A22 = halr_from_adaptive(@(i,j) Afun(m1 + i, n1 + j), ...
            m - m1, n - n1, maxrank, ...
